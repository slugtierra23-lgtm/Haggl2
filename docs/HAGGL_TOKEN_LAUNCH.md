...
This is a checklist for switching HAGGL payments on once the ERC-20 is live
on Base. No code changes are required — everything is gated on environment
variables.

## 1. Deploy the ERC-20

Deploy your HAGGL contract on **Base mainnet (chainId 8453)**. Any standard
...
- USD price per 1 HAGGL at launch (e.g. `0.05` for $0.05 / HAGGL)
...
PLATFORM_WALLET=0x<haggl treasury address — must already be set>
```

That's all the backend needs. It auto-detects ETH vs HAGGL payments per
...
log from `HAGGL_TOKEN_CONTRACT`, it's HAGGL (3% fee, 97% to seller).

### Frontend service

```
NEXT_PUBLIC_HAGGL_TOKEN_CONTRACT=0x<same address as backend>
NEXT_PUBLIC_HAGGL_USD_PRICE=0.05
...
missing, the Pay-with-HAGGL option is hidden and users only see ETH.
...
1. Open any locked repo detail page. You should see a "Pay with ETH · 7%
   fee / Pay with HAGGL · 3% fee" toggle above the Unlock button.
2. Switch to HAGGL, click Unlock. MetaMask should prompt for an ERC-20
   `transfer(seller, amount)` call to the HAGGL contract — value 0 ETH.
3. Confirm. A second prompt sends the 3% platform fee (also HAGGL).
4. Verify the purchase lands in `/orders`, `/inventory → Purchased`, the
   live ticker shows `"currency": "HAGGL"`, and the seller gets rays.

## 5. Updating the USD price

Market price moves. When you need to re-peg the quote:

1. Update `NEXT_PUBLIC_HAGGL_USD_PRICE` on the frontend Render service.
2. Redeploy (~1 min).
...
quotes just mean the buyer pays slightly more/less HAGGL than intended;
no broken purchases.
...
the `usdPrice` read with a cached fetch from `/api/v1/chart/haggl-price`
(not yet implemented; add on the backend when needed).

## Rollback

Want to disable HAGGL payments temporarily? Unset
`NEXT_PUBLIC_HAGGL_TOKEN_CONTRACT` on the frontend. The toggle
disappears, existing HAGGL purchases remain verified in the DB.
