 ...
 * form `haggl-seed:<slug>` so re-runs don't duplicate and we don't
...
const SEED: SeedListing[] = [
  ...
  {
    ownerUsername: 'logic',
    title: 'HagglGuard Pro: AI security scanner for any agent code',
    description:
      "Drop a script, a repo URL, or a webhook into HagglGuard and get a 0-100 security score in under 30 seconds. Detects hardcoded secrets, hidden network calls, shell injection, prompt injection sinks, and unsafe deserialization. Powered by the same LLM-driven scanner that gates every agent listing on haggl.\n\nBuilt for: developers shipping AI agents who don't want to read a security report at 3 AM after their endpoint gets pwned.\n\nWhat you get: 100 free scans per month, JSON report download, security badge for your repo.",
    type: ListingType.AI_AGENT,
    priceEth: 0.002,
    tags: ['security', 'scanner', 'hagglguard', 'audit', 'ai'],
    agentUrl: 'https://www.haggl.tech/hagglguard',
  },
  {
    ownerUsername: 'logic',
    title: 'Webhook starter: Express + TypeScript agent template',
    description:
      'Production-ready Express + TypeScript template for any AI agent that needs a webhook on haggl. Includes: health check endpoint, structured logging, OpenAI/Anthropic adapter, HagglGuard-compatible response shape. Deploy to Render in 1 command.\n\nBuilt for: devs who want to ship an agent in 1 evening, not 1 week.\n\nWhat you get: GitHub repo (private mirror granted on purchase) + 1 hour of Discord support.',
  ...
  },
  {
    ...
    description:
      "Webhook that takes your agent's name, recent on-chain activity, and brand voice, and returns 3 tweet drafts ready to post. Plugs into the haggl Launch tweet flow.\n\nBuilt for: agent owners who launched a token but freeze every time they have to actually tweet about it.",
    type: ListingType.AI_AGENT,
    priceEth: 0.0008,
    tags: ['ai', 'social', 'tweet', 'launch', 'composer'],
    agentEndpoint: 'https://api.haggl.tech/api/v1/agents/tweet-composer',
  },
  ...
  {
    ownerUsername: 'drbug',
    title: 'New Launch Sniper: First-trade alert for haggl Launchpad',
    description:
      'Telegram bot that pings you the second a new token launches on haggl Launchpad. Filters by creator reputation, initial liquidity, and tags. Skip the doom-scrolling, get straight to the launches that matter.\n\nBuilt for: traders who want first-mover position on agent-launched tokens.',
    type: ListingType.BOT,
    priceEth: 0.0008,
    tags: ['bot', 'telegram', 'launchpad', 'sniper', 'alerts'],
    agentEndpoint: 'https://api.haggl.tech/api/v1/bots/launch-sniper',
  },
  {
    ownerUsername: 'drbug',
    title: '1-hour repo audit: structure, security, monetization',
    description:
      "Send your repo URL, get back a 1-page report covering: code structure (red flags, dead deps), security (HagglGuard scan + manual review of the top findings), and monetization angles (which parts could be split into paid agents on haggl). Delivered in under 1 hour.\n\nBuilt for: solo devs sitting on a half-shipped project not sure if it's worth the next 100 hours.",
...
const SEED_REPOS: SeedRepo[] = [
  // ── (A) Founder paid repos ────────────────────────────────────────
  {
    ...
    fullName: 'logic-haggl/next-base-auth-starter',
    description:
    ...
    githubUrl: 'https://github.com/logic-haggl/next-base-auth-starter',
    lockedPriceUsd: 4,
  },
  {
    ownerUsername: 'logic',
    name: 'haggl-agent-boilerplate-ts',
    fullName: 'logic-haggl/haggl-agent-boilerplate-ts',
    description:
      'Production-ready TypeScript boilerplate for any AI agent that needs a haggl webhook. Includes /sell, /chat, /healthz routes, OpenAI + Anthropic adapters, structured logging, HagglGuard-compatible response shape, and a one-command Render deploy.',
    language: 'TypeScript',
    topics: ['haggl', 'agent', 'webhook', 'starter', 'typescript'],
    githubUrl: 'https://github.com/logic-haggl/haggl-agent-boilerplate-ts',
    lockedPriceUsd: 5,
  },
  {
    ownerUsername: 'logic',
    name: 'solidity-escrow-tests',
    fullName: 'logic-haggl/solidity-escrow-tests',
    description:
      'Solidity escrow contract for haggl-style marketplaces, with full Hardhat + Foundry test suite. Buyer deposits, seller delivers, buyer confirms or disputes, 14-day auto-release. Battle-tested patterns; gas-optimized for Base.',
    language: 'Solidity',
    topics: ['solidity', 'escrow', 'base', 'hardhat', 'foundry'],
    githubUrl: 'https://github.com/logic-haggl/solidity-escrow-tests',
    lockedPriceUsd: 6,
  },
  {
    ownerUsername: 'mintak',
    name: 'discord-mod-bot-starter',
    fullName: 'mintak-haggl/discord-mod-bot-starter',
    description:
    ...
    githubUrl: 'https://github.com/mintak-haggl/discord-mod-bot-starter',
    lockedPriceUsd: 3,
  },
  {
    ownerUsername: 'mintak',
    name: 'tg-crypto-alert-bot',
    fullName: 'mintak-haggl/tg-crypto-alert-bot',
    description:
    ...
    githubUrl: 'https://github.com/mintak-haggl/tg-crypto-alert-bot',
    lockedPriceUsd: 3,
  },
  {
    ownerUsername: 'drbug',
    name: 'hagglguard-ci-workflow',
    fullName: 'drbug-haggl/hagglguard-ci-workflow',
    description:
      'Drop-in `.github/workflows/hagglguard.yml` that runs the HagglGuard security scanner on every PR. Comments findings inline, fails the build below your configured threshold, posts the score badge to the README. Works with any TS/JS/Python repo.',
    language: 'YAML',
    topics: ['hagglguard', 'ci', 'github-actions', 'security', 'scanner'],
    githubUrl: 'https://github.com/drbug-haggl/hagglguard-ci-workflow',
    lockedPriceUsd: 2,
  },
  ...
  // ── (B) Curated free starters ─────────────────────────────────────
  {
    ...
    description:
      'Curated by haggl. The official Next.js App Router example from Vercel — production-ready React framework with file-based routing, server components, and edge runtime. Original: https://github.com/vercel/next.js (MIT). Free download, no monetization.',
    language: 'TypeScript',
  ...
  },
  {
    ...
    description:
      'Curated by haggl. Vite + React + TypeScript template — instant dev server, sub-second HMR, ES modules native. Original: https://github.com/vitejs/vite (MIT). Use as the React project base when you do not need Next.js.',
    language: 'TypeScript',
  ...
  },
  {
    ...
    description:
      'Curated by haggl. The Hardhat boilerplate — Solidity dev environment with TypeScript, ethers v6, and a sample ERC20. Original: https://github.com/NomicFoundation/hardhat-boilerplate (MIT). The fastest way to start a smart-contract project for Base.',
    language: 'Solidity',
  ...
  },
  {
    ...
    description:
      'Curated by haggl. Full-stack FastAPI + React + Postgres template. JWT auth, Docker compose, Alembic migrations, automatic OpenAPI docs. Original: https://github.com/tiangolo/full-stack-fastapi-template (MIT). The Python-side counterpart to our Next.js starter.',
    language: 'Python',
  ...
  },
  {
    ...
    description:
      'Curated by haggl. Official Anthropic Claude SDK cookbook — runnable notebooks for tool use, prompt caching, vision, agents, RAG. Original: https://github.com/anthropics/anthropic-cookbook (MIT). Required reading before you build a Claude-powered haggl agent.',
    language: 'Python',
  ...
  },
  {
    ...
    description:
      'Curated by haggl. OpenAI cookbook — recipes for GPT-4, function calling, streaming, embeddings, fine-tuning. Original: https://github.com/openai/openai-cookbook (MIT). Pair with the Anthropic cookbook to keep your model layer pluggable.',
    language: 'Python',
  ...
  },
  {
    ...
    description:
      'Curated by haggl. Official discord.js v14 guide and example bots — slash commands, buttons, modals, voice, sharding. Original: https://github.com/discordjs/guide (MIT). Pair with our paid moderation bot starter for a full Discord stack.',
    language: 'JavaScript',
  ...
  },
  {
    ...
    description:
      'Curated by haggl. The reference Telegram bot library for Python — async, type-hinted, examples for every API surface. Original: https://github.com/python-telegram-bot/python-telegram-bot (LGPL/MPL). Foundation for any Telegram-side automation you build on haggl.',
    language: 'Python',
  ...
  },
  {
    ...
    fullName: 'wevm/viem',
    description:
      'Curated by haggl. Viem — TypeScript interface for Ethereum. Read contracts, sign messages, watch events, simulate calls. Original: https://github.com/wevm/viem (MIT). What we use under the hood for every wallet-touching path on haggl.',
    language: 'TypeScript',
  ...
  },
  {
    ...
    description:
      'Curated by haggl. Foundry — fast Solidity development toolkit (forge, cast, anvil) written in Rust. Original: https://github.com/foundry-rs/foundry (MIT/Apache-2.0). Run alongside Hardhat for the best of both worlds.',
    language: 'Rust',
...
export class SeedListingsService implements OnApplicationBootstrap {
  ...
  private async run(): Promise<void> {
    ...
    for (const item of SEED) {
      // Hard-skip anything mentioning "haggl" — the platform rebranded to
      ...
      if (blob.includes('haggl')) continue;
      ...
      await this.prisma.marketListing.create({
        data: {
          ...
          // Founder listings skip the HagglGuard gate — they have no
          // uploaded code (no fileKey) and the seller is trusted by
          // virtue of being the platform team.
          scanPassed: true,
          scanNote: 'Founder listing — skipped HagglGuard scan (no fileKey to scan).',
  ...
  private async runRepos(): Promise<void> {
    ...
    for (const item of SEED_REPOS) {
      ...
      const githubRepoId = `haggl-seed:${item.fullName.toLowerCase()}`;
